...
print(x[1:])
print(x[:, 1:2, :1])
print(x[0][1][0], x[1][1][0])                         # 索引取值会降维
print(x[0, 1, 0], x[1, 1, 0])                         # 与上面的写完功能是一样的
...
# print(a[0, 2, 4])                                     # 注意这个取值是不能取的，这里在三个维度取值，但是这里只有一个维度
print("--", a[[0,2,4]])                                 # 这里第二个[] 中的就是索引值
...
print("--------------------------------------------------------------")
'小数操作'
a = np.floor(3.5)                                       # 向下取整  地板
b = np.ceil(3.5)                                        # 向上取整  天花板
c = np.int(3.6)                                         # 强转
d = np.pi                                               # PI
e = np.round(d,3)                                       # 四舍五入  保留几位有效数字
f = np.around(d,3)                                      # 四舍五入  保留几位有效数字
g1 = np.rint(3.4)                                       # 四舍五入
g2 = np.rint(3.5)                                       # 四舍五入
